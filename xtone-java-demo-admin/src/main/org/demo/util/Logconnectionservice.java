...
/**
 */
public class Logconnectionservice {

  private static Logconnectionservice instance = new Logconnectionservice();

  private Logconnectionservice() {
  }

  public static Logconnectionservice getInstance() {
    return instance;
  }
  ...
  public static DataSource setupDataSource(String db, int initialSize, int maxActive, int maxIdle, int minIdle) {
    ...
    ds.setUrl(ConfigManager.getConfigData(db + ".url2"));
    ds.setUsername(ConfigManager.getConfigData(db + ".user2"));
    ds.setPassword(ConfigManager.getConfigData(db + ".password2"));
