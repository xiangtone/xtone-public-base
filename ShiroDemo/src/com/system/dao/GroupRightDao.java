...
public class GroupRightDao {
	...
	// Logger.getLogger("GroupRightDao.class");
	...
	public Map<String, Object> load(int pageIndex, String group) {
		...
		String sql = "SELECT " + Constant.CONSTANT_REPLACE_STRING + " "
				+ " FROM `tbl_group_group` a LEFT JOIN `tbl_group` b ON a.`group_id`=b.`id` where 1=1 " + query
		...
		result.put("list", control
				.query(sql.replace(Constant.CONSTANT_REPLACE_STRING, " a.`id`,b.`name`,a.`group_list` list,a.`remark` ")
	...
	public GroupRightModel loadById(int id) {
		...
		String sql = "SELECT a.`group_id` groupid,a.`group_list` list,a.remark "
				+ " FROM `tbl_group_group` a LEFT JOIN `tbl_group` b ON a.`group_id`=b.`id` where a.id=" + id;
	...
	public String loadNameById(int id) {
		String sql = "SELECT name FROM `tbl_group` WHERE id=" + id;
	...
	public int loadIdByName(String name) {
		String sql = "SELECT id FROM `tbl_group` WHERE name LIKE '%" + name + "%'";
	...
	public List<GroupRightModel> loadGroup() {
		String sql = "SELECT id,NAME FROM `tbl_group`";
	...
	public boolean addGroupRight(GroupRightModel model) {
		String sql = "INSERT INTO `tbl_group_group`(`group_id`,`group_list`,`remark`) " + "VALUE(" + model.getGroupId()
				+ ",'" + model.getGroupList() + "','" + model.getRemark() + "')";

		return new JdbcControl().execute(sql);
	}

	public boolean updateGroup(GroupRightModel model) {
		String sql = "UPDATE `tbl_group_group` SET " + "`group_id`=" + model.getGroupId() + "," + "`group_list`='"
				+ model.getGroupList() + "'," + "`remark`='" + model.getRemark() + "' WHERE id=" + model.getId();
		return new JdbcControl().execute(sql);
	}

	public boolean deleteGroup(int id) {
		String sql = "DELETE FROM `tbl_group_group` WHERE id=" + id;
		return new JdbcControl().execute(sql);
	}
