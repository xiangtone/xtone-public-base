...
public class Menu2Dao {
	...
	public Map<String, Object> loadMenu2(int menuHeadId, int menu1Id, int pageIndex) {
		...
		String sql = "select " + Constant.CONSTANT_REPLACE_STRING + " from tbl_menu_2 a left join tbl_menu_1 b "
				+ "on a.menu_1_id = b.id left join tbl_menu_head c on b.head_id = c.id where 1=1 ";
		...
		map.put("rows", new JdbcControl().query(sql.replace(Constant.CONSTANT_REPLACE_STRING, " count(*) ") + query,
				new QueryCallBack() {
					@Override
					public Object onCallBack(ResultSet rs) throws SQLException {
						if (rs.next())
		...
		map.put("list", new JdbcControl().query(
				sql.replace(Constant.CONSTANT_REPLACE_STRING, params) + query + sort + limit, new QueryCallBack() {
					@Override
					public Object onCallBack(ResultSet rs) throws SQLException {
						List<Menu2Model> list = new ArrayList<Menu2Model>();
	...
	public Menu2Model loadMenu2ById(int id) {
		String sql = "select a.id menu2Id,a.name menu2Name,b.id menu1Id,b.name menu1Name,"
				+ "c.id menuHeadId,c.name menuHeadName,a.url,a.action_url from tbl_menu_2 a left join tbl_menu_1 b "
				+ "on a.menu_1_id = b.id left join tbl_menu_head c on b.head_id = c.id where a.id = " + id;
	...
	public void addMenu2(Menu2Model model) {
		String sql = "insert into tbl_menu_2(menu_1_id,name,url,action_url)value(" + model.getMenu1Id() + ",'"
				+ model.getName() + "','" + model.getUrl() + "','" + model.getActionUrl() + "')";
		new JdbcControl().execute(sql);
	}

	public void updateMenu2(Menu2Model model) {
		String sql = "update tbl_menu_2 set menu_1_id = " + model.getMenu1Id() + " ,name = '" + model.getName()
				+ "',url = '" + model.getUrl() + "',action_url = '" + model.getActionUrl() + "' where id ="
				+ model.getId();
		new JdbcControl().execute(sql);
	}

	public boolean updateMenu2(int id, int sort) {
		String sql = "update tbl_menu_2 set sort=" + sort + " where id=" + id;
