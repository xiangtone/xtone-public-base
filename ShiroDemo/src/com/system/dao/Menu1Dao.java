...
public class Menu1Dao {
	private final static Logger LOG = Logger.getLogger("Menu1Dao.class");
	...
	public Map<String, Object> loadMenu1(int menuHeadId, int pageIndex) {
		String sql = "select " + Constant.CONSTANT_REPLACE_STRING
				+ " from tbl_menu_1 a left join tbl_menu_head b on a.head_id = b.id ";
		String query = "";
		if (menuHeadId > 0)
		...
		map.put("list",
				new JdbcControl().query(
						sql.replace(Constant.CONSTANT_REPLACE_STRING, " *,b.name head_name ") + query + limit,
						new QueryCallBack() {
							@Override
							public Object onCallBack(ResultSet rs) throws SQLException {
								List<Menu1Model> list = new ArrayList<Menu1Model>();
	...
	public Map<String, Object> loadMenu1(int menuHeadId, int pageIndex, int menu1Id) {
		String sql = "select " + Constant.CONSTANT_REPLACE_STRING
				+ " from tbl_menu_1 a left join tbl_menu_head b on a.head_id = b.id WHERE 1=1 ";
		String query = "";
		if (menuHeadId > 0)
	...
	public Menu1Model loadMenu1ModelById(int id) {
		String sql = "select * from tbl_menu_1 where id = " + id;
	...
	public boolean updateMenu1Model(Menu1Model model) {
		String sql = "update tbl_menu_1 set head_id = " + model.getMenuHeadId() + ", name = '" + model.getName()
	...
	public boolean updateMenu1Model(int id, int sort) {
		String sql = "update tbl_menu_1 set sort =" + sort + " where id=" + id;

		return new JdbcControl().execute(sql);
	}

	public boolean addMenu1Model(Menu1Model model) {
		String sql = "insert into tbl_menu_1 (head_id,name,remark) value(" + model.getMenuHeadId() + ",'"
