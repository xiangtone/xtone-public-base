...
public class SystemUserDao {
	private final static Logger LOG = Logger.getLogger("SystemUser.class");
	...
	public Map<String, Object> loadUser(int pageIndex, int groupId, String userName, String nickName, int userId) {
		String sql = "select " + Constant.CONSTANT_REPLACE_STRING
				+ " FROM  `tbl_group_user` a LEFT JOIN  tbl_user b ON a.`user_id` = b.`id` where a.create_user="
				+ userId;

		String query = "";

		String sql2 = "select " + Constant.CONSTANT_REPLACE_STRING
				+ " FROM tbl_user a LEFT JOIN `tbl_user` b ON b.`id`=a.`create_user` where a.create_user=" + userId;
				...
				new QueryCallBack() {
					...
					public Object onCallBack(ResultSet rs) throws SQLException {
						...
						while (rs.next()) {
							...
							model.setNick_name(StringUtil.getString(rs.getString("nick_name"), ""));
							...
							model.setCreate_user(rs.getString("group_name"));
	...
	public String loadUserGroup(int userId) {
		String sql = "SELECT b.`group_list` FROM `tbl_group_user` a "
				+ " LEFT JOIN `tbl_group_group` b ON b.`group_id`=a.`group_id` " + " WHERE a.`user_id`=" + userId;
	...
	public boolean adduser(SystemUserModel model) {
		String sql = "INSERT INTO `tbl_user`(`name`,`pwd`,`mail`,`qq`,`phone`,`create_user`,`status`) " + " VALUE('"
				+ model.getName() + "','" + model.getPwd() + "'," + "'" + model.getMail() + "','" + model.getQq() + "',"
	...
	public boolean updateUser(SystemUserModel model) {
		String sql = "UPDATE `tbl_user` SET " + "name='" + model.getName() + "'," + "nick_name='" + model.getNick_name()
				+ "'," + "pwd='" + model.getPwd() + "'," + "qq='" + model.getQq() + "'," + "phone='" + model.getPhone()
				+ "'," + "mail='" + model.getMail() + "'," + "status=" + model.getStatus() + " WHERE id="
				+ model.getId();
		return new JdbcControl().execute(sql);
	}
